...
2.  Install and load new packages you'll need: `car`, `apa`, `effectsize`, and `report`.
...
lever_presses %>% mutate(expected = chi2_lever$expected, 
                         ...
                         residuals_standardized = chi2_lever$stdres)
```
...
-  It is often a good idea to also report means and standard deviations per group (`summarize` with `.by` argument)
...
sum_dat <- dat %>%
  summarize(mean = mean(intrusions),
            sd = sd(intrusions),
            se = se(intrusions), #se = confintr::se_mean(intrusions),
            .by = Condition)
print(sum_dat)
```
