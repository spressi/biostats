...
1.  Within your biostats project in RStudio, create a new script called `DataVisualisation1.R`.
...
```{r}
#| echo: true
#| code-line-numbers: "5-13|4|14-16"
summarydata2 <- 
  summarydata %>% 
  mutate(
...
-   Adding `fill` to the first layer will separate the data into each level of the grouping variable and fill it with a different color. <span style="font-size: 16px;">Note that `fill` colors the inside of the bar, while `color` colors the bar's outlines.</span>
...
The free book [R for data science](https://r4ds.hadley.nz/) was written by (some of) the creators of the `tidyverse`, and includes lots of plotting and data science exercises. (For more advanced plotting, check the [ggplot2 book](https://ggplot2-book.org/).)
